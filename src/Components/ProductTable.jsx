...
export default function ProductTable({ cart, updateCart }) {
	...
	const sortProducts = () => {
		// finds the indexes of the respective sort types
		let indexOfPrice = sortOptions
			.map((object) => object.name)
			.indexOf("Price");
		let indexOfNewest = sortOptions
			.map((object) => object.name)
			.indexOf("Newest");

		// if sort by price is selected
		if (sortOptions[indexOfPrice].current) {
			// sorts products by price into descending order
			setProducts(
				products.sort((a, b) => {
		...
		// sort by newest
		else if (sortOptions[indexOfNewest].current) {
			// sorts products based on their release date, descending order
			setProducts(
				products.sort((a, b) => {
		...
		// default sort
			...
			// sorts products based on their id's given on the first fetch of data
			setProducts(
				products.sort((a, b) => {
					return a.id - b.id;
				})
