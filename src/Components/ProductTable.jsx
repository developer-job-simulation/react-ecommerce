  ...
  return (
    <div className="bg-white">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <ProductFilters
          {...{ filterOptions, setFilterOptions, sortOptions, setSortOptions, getDefaultFilterOptions }}
        />

        <div className="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
          {products
            .sort((x, y) => {
              let activeSortings = sortOptions.filter(({ current }) => current);
              if (activeSortings.length > 0) {
                if (activeSortings[0].name === "Price") {
                  return x.price - y.price;
                } else {
                  return Number(x.releaseDate) - Number(y.releaseDate);
                }
              }
            })
            .map((product) => (
