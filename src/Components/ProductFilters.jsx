...
export default function ProductFilters({
	...
	const [noFilters, setNoFilters] = useState(0);

	// toggle the filter
	const toggleFilter = (type, filterNo) => {
		let filterOptionsCopy = { ...filterOptions };

		// toggles the filter's checked value
		filterOptionsCopy[type][filterNo].checked =
			!filterOptionsCopy[type][filterNo].checked;
								...
								{filterOptions.price.map((option, optionIdx) => (
									...
									>
										<input
											...
											onChange={(e) => {
												// if filter is selected, increment 1
												if (e.target.checked) {
													setNoFilters((prev) => prev + 1);
													toggleFilter("price", optionIdx);
												} else {
													setNoFilters((prev) => prev - 1);
													toggleFilter("price", optionIdx);
												}
